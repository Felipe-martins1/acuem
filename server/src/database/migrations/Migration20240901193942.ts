...
export class Migration20240901193942 extends Migration {
  override async up(): Promise<void> {
    ...
    this.addSql(
      'create table "university" ("id" serial primary key, "name" varchar(255) not null, "address_street" varchar(255) not null, "address_neighborhood" varchar(255) not null, "address_complement" varchar(255) not null, "address_number" varchar(255) not null, "address_city" varchar(255) not null, "address_state" varchar(255) not null);',
    );

    this.addSql(
      'create table "store" ("id" serial primary key, "name" varchar(255) not null, "cnpj" varchar(14) not null, "university_id" int not null, "address_street" varchar(255) not null, "address_neighborhood" varchar(255) not null, "address_complement" varchar(255) not null, "address_number" varchar(255) not null, "address_city" varchar(255) not null, "address_state" varchar(255) not null);',
    );
    ...
    this.addSql(
      'create table "product" ("id" serial primary key, "name" varchar(255) not null, "description" varchar(255) not null, "price" numeric(10,0) not null, "quantity" int not null default 0, "active" boolean not null default true, "category_id" int not null, "store_id" int not null);',
    );
    ...
    this.addSql(
      'create table "user" ("id" uuid not null default gen_random_uuid(), "username" varchar(255) not null, "email" varchar(255) not null, "phone" varchar(15) not null, "name" varchar(255) not null, "password" varchar(255) not null, "type" text check ("type" in (\'student\', \'employee\')) not null, "created_at" timestamptz not null, "store_id" int null, "university_course_id" int null, "points" int null default 0, constraint "user_pkey" primary key ("id"));',
    );
    this.addSql('create index "user_type_index" on "user" ("type");');

    this.addSql(
      'create table "purchase" ("id" serial primary key, "date" timestamp not null, "status" text check ("status" in (\'pending\', \'confirmed\', \'rejected\', \'started\', \'finished\', \'received\')) not null default \'pending\', "finished_at" timestamp null, "cancel_at" timestamp null, "cancel_cause" varchar(255) null, "student_id" uuid not null, "store_id" int not null);',
    );

    this.addSql(
      'create table "purchase_product" ("id" serial primary key, "purchase_id" int not null, "product_id" int not null, "quantity" int not null, "current_price" numeric(10,0) not null);',
    );

    this.addSql(
      'alter table "store" add constraint "store_university_id_foreign" foreign key ("university_id") references "university" ("id") on update cascade;',
    );
